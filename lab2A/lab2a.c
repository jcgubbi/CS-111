...
int main(int argc, char *argv[]){
	...
	while( (i = getopt_long(argc, argv, "", args, NULL)) != -1){
		if(profile){
			timestatus = getrusage(RUSAGE_SELF, &beg);
			if(timestatus < 0){
				fprintf(stderr,"Error with getrusage - %s", strerror(errno));
				exitcode = 1;
			}
		}
		switch(i){
		...
		if(profile){
                        timestatus = getrusage(RUSAGE_SELF, &fin);
                        if(timestatus<0){
				exitcode = 1;
				fprintf(stderr,"Error with getrusage - %s",strerror(errno)); 
			}
                        printf("user time: %f ",(fin.ru_utime.tv_sec - beg.ru_utime.tv_sec) + (float)(fin.ru_utime.tv_usec - beg.ru_utime.tv_usec)/1000000);
                        printf("system time: %f\n",(fin.ru_stime.tv_sec - beg.ru_stime.tv_sec) + (float)(fin.ru_stime.tv_usec - beg.ru_stime.tv_usec)/1000000);
                }
	}
	if(profile && wait){
		timestatus = getrusage(RUSAGE_CHILDREN, &fin);
		if(timestatus<0 && exitcode == 0){
			exitcode = 1;
		}
		printf("user time: %f ",fin.ru_utime.tv_sec + (float)fin.ru_utime.tv_usec/1000000);
		printf("system time: %f\n",fin.ru_stime.tv_sec + (float)fin.ru_stime.tv_usec/1000000);
	}
	if(highestsignal>0){
